...
class BP_Core_User {
	...
	function get_specific_users( $user_ids, $limit = null, $page = 1, $populate_extras = true ) {
		...
		$total_users_sql = apply_filters( 'bp_core_get_specific_users_count_sql', $wpdb->prepare( "SELECT COUNT(ID) FROM {$wpdb->users} WHERE {$status_sql} AND ID IN ( " . $wpdb->escape( $user_ids ) . " ) " ), $wpdb->escape( $user_ids ) );
