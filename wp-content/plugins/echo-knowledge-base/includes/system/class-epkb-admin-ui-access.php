...
class EPKB_Admin_UI_Access {
	...
	public static function is_user_access_to_context_allowed( $context ) {
		...
		// FAQs, Quizzes, and Glossary are not specific to KB so use default KB ID
		if ( in_array( $context, array( 'admin_eckb_access_faqs_write', 'admin_eckb_access_quizzes_write', 'admin_eckb_access_glossary_write' ), true ) ) {
	...
	public static function get_context_required_capability( $contexts, $kb_config=null ) {
		...
		// FAQs, Quizzes, and Glossary are not specific to KB so use default KB ID
		if ( isset( $contexts[0] ) && in_array( $contexts[0], array( 'admin_eckb_access_faqs_write', 'admin_eckb_access_quizzes_write', 'admin_eckb_access_glossary_write' ), true ) ) {
			$config = epkb_get_instance()->kb_config_obj->get_kb_config_or_default( EPKB_KB_Config_DB::DEFAULT_KB_ID );
	...
	public static function get_access_boxes( $kb_config ) {
		...
		$boxes_config[] = $kb_id == EPKB_KB_Config_DB::DEFAULT_KB_ID ?
			array(
				'title' => $kb_config_specs_labels['admin_eckb_access_glossary_write'],
				'description'   => esc_html__( 'The Glossary feature is not linked to any specific KB; instead, access to it is defined within the default KB.', 'echo-knowledge-base' ),
				'html' => self::radio_buttons_vertical_access_control( array(
					'name'          => 'admin_eckb_access_glossary_write',
					'radio_class'   => 'epkb-admin__radio-button-wrap',
					'return_html'   => true,
					'value'         => self::is_capability_in_allowed_list( $kb_config['admin_eckb_access_glossary_write'], $kb_id )
