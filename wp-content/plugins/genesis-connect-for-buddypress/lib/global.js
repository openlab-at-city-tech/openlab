		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				if ( 0 != jq("#latest-update").length ) {
					...
					u += '<a href="' + v + '" rel="nofollow">' + Connect.view + '</a>';
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			function(response) {
				...
				target.fadeOut( 100, function() {
					...
					jq(this).attr('title', 'fav' == type ? Connect.remove_fav : Connect.mark_as_fav);
					jq(this).fadeIn(100);
				});

				if ( 'fav' == type ) {
					if ( !jq('.item-list-tabs li#activity-favorites').length )
						jq('.item-list-tabs ul li#activity-mentions').before( '<li id="activity-favorites"><a href="#">' + Connect.my_favs + ' <span>0</span></a></li>');
...
function Connect_hide_comments() {
