...
(function($, window, document, undefined) {
  ...
  $.fn.themeSidebarMenu = function() {
        ...
        $(
          '#' +
            $(this)
              .attr('id')
              .replace(/submit-/, '')
        ).addSelectedToSidebar(api.addMenuItemToBottom);
      });
    ...
    api.initAccordion = function() {
      ...
      $('.accordion-section-content input[type="checkbox"]').prop(
        'checked',
        false
      );
    ...
    api.initPagination = function(panelLink) {
      ...
      $(panelLink).on('click', function(e) {
        ...
        $.post(
          ajaxurl,
          e.target.href.replace(/.*\?/, '').replace(/action=([^&]*)/, '') +
            '&action=ecs_sidebar_get_metabox',
          function(resp) {
            ...
            panelLink =
              '#' + metaBoxData['replace-id'] + ' .add-menu-item-pagelinks a';
      ...
      $.fn.extend({
        ...
        addSelectedToSidebar: function(processMethod) {
          ...
          return this.each(function() {
            ...
            var menuItems = {};
            var checkboxes =
              sidebarsL10n.oneThemeLocationNoSidebars &&
              0 ===
    ...
    api.updateQuickSearchResults = function(input) {
      ...
      params = {
        ...
        menu: $('#menu').val(),
        ecs_sidebar_quick_search_nonce: $(
          '#ecs_sidebar_quick_search_nonce'
        ).val(),
        q: q,
    ...
    api.processQuickSearchQueryResponse = function(resp, req, panel) {
      ...
      if (!$items.length) {
        $('.categorychecklist', panel).html(
          '<li><p>' + sidebarsL10n.noResultsFound + '</p></li>'
        );
        $('.spinner', panel).toggleClass('ecs-visible');
        // $('.spinner', panel).hide();
        return;
      ...
      $items.each(function() {
        ...
        if (matched && matched[1]) {
          newID = matched[1];
          while (
            form.elements['sidebar-item[' + newID + '][sidebar-item-type]'] ||
            takenIDs[newID]
          ) {
            newID--;
          }
          ...
          if (newID != matched[1]) {
            $item.html(
              $item
                .html()
                .replace(
                  new RegExp('sidebar-item\\[' + matched[1] + '\\]', 'g'),
                  'sidebar-item[' + newID + ']'
                )
            );
          }
    ...
    api.initSortables = function() {
      ...
      $('#sidebar-to-edit').sortable({
        ...
        sort: function(event, ui) {
          $('#sidebar-to-edit .sortable-placeholder').height(
            $('.ui-sortable-helper').height() - 2
          );
        },
    ...
    api.registerEditEvents = function() {
      ...
      $(
        '#custom-sidebar-name, #sidebar_replacement_id, #sidebar_description'
      ).on('change', function() {
        api.registerChange();
      });

      // Disable metaboxes on create screen
      $('.metabox-holder-disabled input[type="submit"]').attr(
        'disabled',
        'disabled'
      );
      ...
      $('#create_sidebar_header, #create_sidebar_footer').on(
        'click',
        function() {
    ...
    api.registerManagementEvents = function() {
      ...
      $('#sidebar-replacements-table a.sidebar-delete-link').on(
        'click',
        function() {
          var confirmation = confirm(sidebarsL10n.deleteWarning);
          var spinner = $(this)
      ...
      $('#sidebar-replacements-table select').on('change', function() {
        ...
        $.post(ajaxurl, dataObj, function() {}).done(function() {
          ...
          if ('0' === replacementId) {
            select
              ...
              .html(sidebarsL10n.activateSidebar);
          } else {
            select
              .children('option[value="0"]')
              .html(sidebarsL10n.deactivateSidebar);
          }
          spinner.fadeOut(200);
        });
      });
