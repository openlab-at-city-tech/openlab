...
//        die();
        ...
        switch($ays_quiz_question_by) {
            case 'category':
                $sql = "SELECT q.question,q.category_id,q.question_image,a.answer,a.image
                    ...
                    $content .= '<div style="margin-bottom:20px;" class="ays_quiz_flash_card_content" data-index="0">';
                        $content .= '<div class="ays_quiz_flash_card_container_'.$cat_key.'">';
                            foreach ($group_by_cat as $key => $result) {
                                ...
                                $ays_quiz_answer   = (isset($result['answer']) && $result['answer'] != '') ? sanitize_text_field($result['answer']) : '';
                        ...
                        if ($question_count != 1) {
                            $content .= '<div class="ays_quiz_fc_next_btn_content">';
                                $content .= '<a class="ays_quiz_fc_next_prev_btn prev">Prev</a>';
                                $content .= '<a class="ays_quiz_fc_next_prev_btn next">Next</a>';
            ...
            default:
                if( !empty( $ays_question_id ) ){
                    foreach ($ays_question_id as $quiz_key => $ays_q_id) {
                        $sql = "SELECT q.question,q.question_image,a.answer,a.image
                                FROM {$answer_table} AS a
                                LEFT JOIN {$question_table} AS q
                                    ON a.question_id = q.id
