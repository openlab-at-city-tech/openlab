...
class Quiz_Orders_List_Table extends WP_List_Table {
    ...
    public static function get_orders( $per_page = 20, $page_number = 1 ) {
        ...
        if(! empty( $_REQUEST['orderby'] ) &&  $_REQUEST['orderby'] == 'quiz_complete' ){
            $quizzes = array();
            $sql = "SELECT {$wpdb->prefix}aysquiz_quizes.id FROM {$wpdb->prefix}aysquiz_quizes INNER JOIN {$wpdb->prefix}aysquiz_orders ON {$wpdb->prefix}aysquiz_quizes.id={$wpdb->prefix}aysquiz_orders.quiz_id";
            ...
            arsort($results);
            foreach ($results as $key=>$result){
                array_push($quizzes,Results_List_Table::get_report_by_quiz_id ($key ));
            }
