...
jq(document).ready( function() {
	...
	jq("input#aw-whats-new-submit").click( function() {
		...
		function(response)
		{
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				setTimeout( function() { jq('form.ac-form').hide(); }, 250 );
			}
		});
			...
			function(response) {
				...
				if ( 'fav' == type ) {
					if ( !jq('div.item-list-tabs li#activity-favorites').length )
						jq('div.item-list-tabs ul li#activity-mentions').before( '<li id="activity-favorites"><a href="#">' + Connect.my_favs + ' (<span>0</span>)</a></li>');
	...
	jq('span.highlight span').click( function() {
		if ( !jq('div.help').length ) {
			jq(this).parent().after( '<div id="message" class="info help"><p>' + Connect.mention_explain + '</p></div>' );
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				button.fadeOut( 100, function() {
					if ( jq(this).hasClass('accept') ) {
						jq(this).html( Connect.accepted ).fadeIn(50);
						jq(this).addClass('accepted');
					} else {
						jq(this).html( Connect.rejected ).fadeIn(50);
	...
	jq('body.internal-page table.zebra tbody tr').mouseover( function() {
		jq(this).addClass('over');
	}).mouseout( function() {
		jq(this).removeClass('over');
	});
		
	jq('body.internal-page table.zebra tbody tr:odd').addClass('alt');
	...
	comments_divs.each( function() {
		...
		comment_lis.each( function(i) {
			...
			if ( i < comment_lis.length - 5 ) {
				...
				if ( !i )
					jq(this).before( '<li class="show-all"><a href="#' + parent_li.attr('id') + '/show-all/" title="' + Connect.show_all_comments + '">' + Connect.show_all + ' ' + comment_count + ' ' + Connect.comments + '</a></li>' );
			}
		});
