...
class BP_Docs_Groups_Integration {
	var $enable_create_step = false; //WDS take out creation
	...
	function show_doc_count_in_tab() {
		...
		if ( !empty( $bp->bp_options_nav[$group_slug] ) && !empty( $bp->bp_options_nav[$group_slug][BP_DOCS_SLUG] ) ) {
			...
			$bp->bp_options_nav[$group_slug][BP_DOCS_SLUG]['name'] = sprintf( __( 'Wiki (%d)', 'bp-docs' ), $doc_count );	
		}
	}
...
class BP_Docs_Group_Extension extends BP_Group_Extension {
	...
	function bp_docs_group_extension() {
		...
		$this->name 			= __( 'Wiki', 'bp-docs' );
		$this->slug 			= BP_DOCS_SLUG;

		$this->enable_create_step	= false; //WDS MOD
	...
	function admin_markup() {
		...
		-->
		<p>
			 <label for="bp-docs[group-enable]"> <input type="checkbox" name="bp-docs[group-enable]" id="bp-docs-group-enable" value="1" <?php checked( $group_enable, true ) ?> /> Enable Wiki</label>
		</p>
		...
		<div id="group-doc-options" <?php if ( !$group_enable ) : ?>class="hidden"<?php endif ?>>
			...
			<table class="group-docs-options">
				<tr>
					<td class="label">
						<label for="bp-docs[can-create-admins]"><?php _e( 'Minimum role to create new wiki doc:', 'bp-docs' ) ?>
					</td>
...
?>
	<li<?php if ( $bp->bp_docs->current_view == 'list' ) : ?> class="current"<?php endif; ?>><a href="<?php echo $bp->root_domain . '/' . $groups_slug ?>/<?php echo $bp->current_item; ?>/<?php echo $bp->bp_docs->slug ?>/"><?php _e( 'View Docs', 'bp-docs' ) ?></a></li>
	<?php if ( bp_docs_current_user_can( 'create' ) ) : ?>
		<li<?php if ( 'create' == $bp->bp_docs->current_view ) : ?> class="current"<?php endif; ?>><a href="<?php echo $bp->root_domain . '/' . $groups_slug ?>/<?php echo $bp->current_item; ?>/<?php echo $bp->bp_docs->slug ?>/create"><?php _e( 'New Doc', 'bp-docs' ) ?></a></li>
	<?php endif ?>	
	
	<?php if ( bp_docs_is_existing_doc() ) : ?>
		<li class="current"><a href="<?php echo $bp->root_domain . '/' . $groups_slug ?>/<?php echo $bp->current_item; ?>/<?php echo $bp->bp_docs->slug ?>/<?php echo $post->post_name ?>"><?php the_title() ?></a></li>		
	<?php endif ?>
