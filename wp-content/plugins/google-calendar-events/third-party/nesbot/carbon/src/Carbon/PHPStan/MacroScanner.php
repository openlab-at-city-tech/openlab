...
final class MacroScanner
{
     ...
     * @return Macro
     */
    public function getMethod(string $className, string $methodName) : \SimpleCalendar\plugin_deps\Carbon\PHPStan\Macro
    {
        $reflectionClass = new ReflectionClass($className);
        $property = $reflectionClass->getProperty('globalMacros');
        $property->setAccessible(\true);
        $macro = $property->getValue()[$methodName];
        return new \SimpleCalendar\plugin_deps\Carbon\PHPStan\Macro($className, $methodName, $macro);
    }
}
