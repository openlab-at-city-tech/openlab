 ...
 */
class Logger implements LoggerInterface, \SimpleCalendar\plugin_deps\Monolog\ResettableInterface
{
    /**
    ...
    public function addRecord(int $level, string $message, array $context = []) : bool
    {
        ...
        foreach ($this->handlers as $handler) {
            if (null === $record) {
                ...
                $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => new \SimpleCalendar\plugin_deps\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                try {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \SimpleCalendar\plugin_deps\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \SimpleCalendar\plugin_deps\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
