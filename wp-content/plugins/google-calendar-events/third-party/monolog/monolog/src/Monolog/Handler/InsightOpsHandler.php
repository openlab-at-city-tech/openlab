 ...
 */
class InsightOpsHandler extends \SimpleCalendar\plugin_deps\Monolog\Handler\SocketHandler
{
    /**
     * @var string
     */
    ...
    public function __construct(string $token, string $region = 'us', bool $useSSL = \true, $level = Logger::DEBUG, bool $bubble = \true)
    {
        if ($useSSL && !\extension_loaded('openssl')) {
            throw new \SimpleCalendar\plugin_deps\Monolog\Handler\MissingExtensionException('The OpenSSL PHP plugin is required to use SSL encrypted connection for InsightOpsHandler');
        }
