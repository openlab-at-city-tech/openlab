 ...
 */
class CubeHandler extends \SimpleCalendar\plugin_deps\Monolog\Handler\AbstractProcessingHandler
{
    ...
    protected function connectUdp() : void
    {
        if (!\extension_loaded('sockets')) {
            throw new \SimpleCalendar\plugin_deps\Monolog\Handler\MissingExtensionException('The sockets extension is required to use udp URLs with the CubeHandler');
        }
    ...
    protected function connectHttp() : void
    {
        if (!\extension_loaded('curl')) {
            throw new \SimpleCalendar\plugin_deps\Monolog\Handler\MissingExtensionException('The curl extension is required to use http URLs with the CubeHandler');
        }
    ...
    private function writeHttp(string $data) : void
    {
        ...
        \SimpleCalendar\plugin_deps\Monolog\Handler\Curl\Util::execute($this->httpConnection, 5, \false);
    }
}
