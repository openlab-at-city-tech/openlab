...
/**
 * Check if TablePress is compatible with the current environment.
 */
if ( file_exists( WP_PLUGIN_DIR . '/freemius-nulled/freemius-nulled.php' ) ) {
	/**
	 * Show an error notice to admins, if TablePress is not compatible with the current environment.
	 *
	 * @since 2.4.0
	 */
	function tablepress_admin_error_notice_incompatible_environment() /* No return type declaration, due to required PHP compatibility! */ {
		?>
		<div class="notice notice-error notice-alt notice-large">
			<h3><em>
				<span aria-hidden="true" class="dashicons dashicons-warning" style="color:#d63638;vertical-align:bottom"></span>
			...
			<p style="font-size:14px">
				<?php _e( 'TablePress is not compatible with the current environment.', 'tablepress' ); ?>
			</p>
			<p style="font-size:14px">
				<strong><?php printf( __( 'Please <a href="%1$s">get a TablePress premium license</a> or <a href="%2$s">contact the TablePress developer</a>.', 'tablepress' ), 'https://tablepress.org/pricing/', 'mailto:support@tablepress.org' ); ?></strong>
			</p>
		</div>
		<?php
	}
	add_action( 'admin_notices', 'tablepress_admin_error_notice_incompatible_environment' );
