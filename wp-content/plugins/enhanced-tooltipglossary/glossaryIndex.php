...
class CMTT_Glossary_Index {
	...
	public static function outputGlossaryIndexPage( $shortcodeAtts ) {
		...
		$sortByTitle = \CM\CMTT_Settings::get( 'cmtt_index_sortby_title' );
		$orderby     = $sortByTitle ? 'title' : 'name';
		...
		if ( $serverSidePagination ) {
			$args['posts_per_page'] = isset( $shortcodeAtts['perpage'] ) ? $shortcodeAtts['perpage'] : \CM\CMTT_Settings::get( 'cmtt_perPage' );
		...
		if ( self::isServerSide() && ! isset( $args['nopaging'] ) && in_array( $paginationPosition, array(
				'top',
				'both'
			) ) ) {
		...
		if ( $glossary_index ) {
			...
			foreach ( $glossary_index as $glossaryItem ) {
				 ...
				 */
				$glossaryItemDesc = ( \CM\CMTT_Settings::get( 'cmtt_glossaryTooltipDesc' ) == 1 ) ? '<dfn class="glossary_itemdesc" role="definition">' . $glossaryItem->post_content . '</dfn>' : '';
				...
				if ( \CM\CMTT_Settings::get( 'cmtt_showFeaturedImageThumbnail', false ) && in_array( $glossaryIndexStyle, array(
						'classic-excerpt',
						'classic-definition'
					) ) ) {
				...
				$preItemTitleContent .= ( in_array( $glossaryIndexStyle, array(
					'term-definition',
					'img-term-definition',
				...
				$preItemTitleContent .= '<' . $tag . ' ' . $href . ' ' . $windowTarget . $rel
				                        . ' role="term" class="' . $glossary_list_class . ' ' . $additionalClass . '" ' . $titleAttr . ' '
				                        . $dataPostName . $styleAttr;
				...
				$postItemTitleContent .= in_array( $glossaryIndexStyle, array(
					'img-term-definition',
					'flipboxes-with-definition'
				) ) ? '</div>' : '';
				...
				if ( ! $hideTerms ) {
					...
					if ( $sortByTitle ) {
						...
						$key = urldecode( $glossaryItem->post_name );
					}
