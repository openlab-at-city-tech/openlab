...
tooltipBottom=document.createElement('div');tooltipBottom.setAttribute('id',id+'bot');if(opts.close_button!==!1&&((opts.close_button_mobile===!0&&CM_Tools.Modernizr.touch)||opts.close_button_mobile===!1)){tooltipButtonClose=document.createElement('span');tooltipButtonClose.setAttribute('id',id+'-btn-close');tooltipButtonClose.setAttribute('class','dashicons '+opts.close_symbol);tooltipButtonClose.setAttribute('aria-label','Close the tooltip');tooltipTop.appendChild(tooltipButtonClose);jQuery(tooltipWrapper).on('click touchstart',function(e){var tooltipVisible=jQuery('#tt').is(':visible');var audioLinkClicked='A'===e.target.tagName&&jQuery(e.target).parents('.cmtt-audio-player').length>=1;closeButtonClicked=jQuery(e.target).attr('id')==='tt-btn-close';if(tooltipVisible&&!audioLinkClicked&&closeButtonClicked){tooltipApi.hide()}})}
...
var linkAddon=Math.floor(Math.random()*(999-100+1))+100;var callBackId=defLinkId+'-'+linkAddon;$(parentSpan).attr('id',callBackId);$(backLinkId).find('a').attr('href','#'+callBackId);$('.cmtt-footnote-def').css('background-color','');var footnoteContentBlock=$('.'+ftid);if(!footnoteContentBlock.length){footnoteContentBlock=$('#'+ftid)}
footnoteContentBlock.css('background-color','#eaf3ff');if(footnoteContentBlock.length){$('html, body').animate({scrollTop:footnoteContentBlock.offset().top-50},500)}};CMTT_Footnote.linkBackClickHandler=function(e){e.preventDefault();var link=$(this);var href=link.attr('href');var backElement=$(href);$('.cmtt-footnote-deflink').css('background-color','');$('.cmtt-footnote-deflink').parent().css('background-color','');$(backElement).find('sup').css('background-color','#eaf3ff');if(backElement.length){$('html, body').animate({scrollTop:backElement.offset().top-100},500)}}
