					...
					function createLettersHtml() {
						...
						for ( var i = 0; i < letters.length; i ++ ) {
							...
							html.push( '<a class="ln-' + (
								letters[i] === '-' ? '-' : i
							) + ' lnletter-' + getLowerLetter( letters[i] ) + '" tabindex="-1" role="tab" aria-selected="false" aria-controls="panel-' + getLowerLetter( letters[i] ) + '" href="#">' + (
								           (
									           letters[i] === '-'
								           ) ? '...' : letters[i].replace( 'ı', 'İ' ).replace( 'i', 'I' ).toUpperCase()
							           ) + '</a>' );
						}
		...
		$( document ).ready(
			function () {
				...
				function showLoading( thiz ) {
					$( thiz ).find( '.glossary-container .progress-indicator' ).css( 'display', 'block' );
				...
				function glossary_filter( thiz, additional_data ) {
					showLoading( thiz );
					var data = {}, base_data = {}, currentSearch, previousSearch, cb_search, searchChanged,
						tile = thiz.closest( '.glossary-container' );
					...
					if ( noAjax ) {
						...
						$.post(
							window.cmtt_listnav_data.ajaxurl,
							data,
							function ( response ) {
								if ( response === '0' ) {
									tile.html( 'Error' );
