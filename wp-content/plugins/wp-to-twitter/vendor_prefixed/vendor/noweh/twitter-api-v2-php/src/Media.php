...
class Media extends AbstractController
{
    ...
    private function prepareRequest(array $settings = []) : void
    {
        ...
        $oAuth1 = new Oauth1(['consumer_key' => $settings['consumer_key'], 'consumer_secret' => $settings['consumer_secret'], 'token' => $settings['access_token'], 'token_secret' => $settings['access_token_secret']]);
        $stack->push($oAuth1);
        $this->client = new Client(['base_uri' => "https://upload.twitter.com/1.1/", 'handler' => $stack, 'auth' => 'oauth']);
    }
    /**
