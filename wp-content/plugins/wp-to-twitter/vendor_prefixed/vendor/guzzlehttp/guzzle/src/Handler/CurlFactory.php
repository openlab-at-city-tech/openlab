...
use WpToTwitter_Vendor\GuzzleHttp\Exception\ConnectException;
use WpToTwitter_Vendor\GuzzleHttp\Exception\RequestException;
use WpToTwitter_Vendor\GuzzleHttp\Promise as P;
use WpToTwitter_Vendor\GuzzleHttp\Promise\FulfilledPromise;
use WpToTwitter_Vendor\GuzzleHttp\Promise\PromiseInterface;
use WpToTwitter_Vendor\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \WpToTwitter_Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \WpToTwitter_Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
