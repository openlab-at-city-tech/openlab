...
namespace WpToTwitter_Vendor\GuzzleHttp\Subscriber\Oauth;

use WpToTwitter_Vendor\GuzzleHttp\Psr7\Query;
...
class Oauth1
{
    ...
    {
        $this->config = ['version' => '1.0', 'request_method' => self::REQUEST_METHOD_HEADER, 'consumer_key' => 'anonymous', 'consumer_secret' => 'anonymous', 'signature_method' => self::SIGNATURE_METHOD_HMAC];
    ...
    {
        return \sha1(\uniqid('', \true) . $request->getUri()->getHost() . $request->getUri()->getPath());
    }
    /**
    ...
    private function signUsingHmac($algo, $baseString)
    {
        ...
        return \hash_hmac($algo, $baseString, $key, \true);
    }
    /**
     * @param string $baseString
     *
    ...
    private function signUsingRsaSha1($baseString)
    {
        ...
        $privateKey = \openssl_pkey_get_private(\file_get_contents($this->config['private_key_file']), $this->config['private_key_passphrase']);
        $signature = '';
    ...
    private function buildAuthorizationHeader(array $params)
    {
        ...
        if (isset($this->config['realm'])) {
            \array_unshift($params, 'realm="' . \rawurlencode($this->config['realm']) . '"');
        }
    ...
    {
        $params = ['oauth_consumer_key' => $config['consumer_key'], 'oauth_nonce' => $nonce, 'oauth_signature_method' => $config['signature_method'], 'oauth_timestamp' => \time()];
        ...
        // service.
        $optionalParams = ['callback' => 'oauth_callback', 'token' => 'oauth_token', 'verifier' => 'oauth_verifier', 'version' => 'oauth_version', 'bodyhash' => 'oauth_body_hash'];
