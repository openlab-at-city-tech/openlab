 ...
 * @since 3.0.0
 *
 * @package \TEC\Common\StellarWP\Schema\Traits;
 */
declare (strict_types=1);
namespace TEC\Common\StellarWP\Schema\Traits;
...
use TEC\Common\StellarWP\Schema\Columns\Contracts\Auto_Incrementable;
...
trait Custom_Table_Query_Methods
{
    ...
    protected static function fetch_all(int $batch_size = 50, string $output = OBJECT, string $where_clause = '', string $order_by = ''): Generator
    {
        ...
        do {
            ...
            $query = $database::prepare("SELECT * FROM %i {$where_clause} ORDER BY {$order_by} LIMIT %d, %d", static::table_name(true), $offset, $batch_size);
            $batch = $database::get_results($query, $output);
    ...
    public static function insert_many(array $entries)
    {
        ...
        $database = Config::get_db();
        return $database::query($database::prepare("INSERT INTO %i ({$prepared_columns}) VALUES {$prepared_values}", static::table_name(true)));
    }
    /**
    ...
    public static function delete_many(array $ids, string $column = '', string $more_where = '')
    {
        ...
        $column = $column ? "{$column} IN ({$prepared_ids})" : implode(' AND ', array_map(function ($c) use ($prepared_ids) {
            return "{$c} IN ({$prepared_ids})";
        }, static::primary_columns()));
        return $database::query($database::prepare("DELETE FROM %i WHERE {$column} {$more_where}", static::table_name(true)));
    }
    /**
    ...
    protected static function prepare_statements_values(array $entries): array
    {
        ...
        $entries = array_map(function ($entry) use ($uid_column, $column_object) {
        ...
        $prepared_values = implode(', ', array_map(static fn(array $entry) => '(' . implode(', ', $entry) . ')', $prepared_values));
    ...
    {
        $database = Config::get_db();
        return $database::get_row($database::prepare("SELECT * FROM %i {$where_clause} LIMIT 1", static::table_name(true)), $output);
    }
    /**
    ...
    public static function get_total_items(array $args = []): int
    {
        ...
        return (int) $database::get_var($database::prepare("SELECT COUNT(*) FROM %i a {$where}", static::table_name(true)));
    }
    /**
    ...
    public static function update_many(array $entries): bool
    {
        ...
        foreach ($entries as $entry) {
            ...
            $queries[] = $database::prepare("UPDATE %i SET {$set_statement} WHERE {$uid_column} = %s;", static::table_name(true), $uid);
        }
    ...
    public static function paginate(array $args, int $per_page = 20, int $page = 1, array $columns = ['*'], string $join_table = '', string $join_condition = '', array $selectable_joined_columns = []): array
    {
        ...
        $database = Config::get_db();
        $results = $database::get_results($database::prepare("SELECT {$formatted_columns}{$secondary_columns} FROM %i a {$join} {$where} ORDER BY a.{$orderby} {$order} LIMIT %d, %d", static::table_name(true), $offset, $per_page), ARRAY_A);
    ...
    protected static function get_join_parts(string $join_table, string $join_condition, array $selectable_joined_columns = []): array
    {
        ...
        return [$database::prepare("JOIN %i b ON {$join_condition}", $join_table::table_name(true)), $clean_secondary_columns];
    }
    /**
    ...
    private static function prepare_value_for_query(string $column, $value): array
    {
        ...
        switch ($column->get_php_type()) {
            ...
            case PHP_Types::DATETIME:
                $value = is_array($value) ? array_map(fn($v) => $v instanceof DateTimeInterface ? $v->format('Y-m-d H:i:s') : (string) $v, $value) : ($value instanceof DateTimeInterface ? $value->format('Y-m-d H:i:s') : (string) $value);
                $placeholder = '%s';
                break;
            case PHP_Types::JSON:
    ...
    {
        return ['eq' => '=', 'neq' => '!=', 'gt' => '>', 'lt' => '<', 'gte' => '>=', 'lte' => '<=', 'in' => 'IN', 'not_in' => 'NOT IN'];
    }
    /**
