...
class B2S_Meta {
    ...
    public function getOgMeta() {
        ...
        $typeChanged=  $this->addWooCommerce();
        if(!$typeChanged){
            $this->getObjectType();
        }
    ...
    public function addWooCommerce() {

        $typeChanged= false;

        // Check if metaData has WooCommerce product fields
        if (!is_array($this->metaData) || empty($this->metaData)) {
            return false;
        }

        // Only output WooCommerce tags if og:type is set to 'product'
        if (isset($this->metaData['og:type']) && $this->metaData['og:type'] == 'product' && isset($this->metaData['og:price:amount']) && !empty($this->metaData['og:price:amount'])&& isset($this->metaData['og:price:currency']) && !empty($this->metaData['og:price:currency'])) {
            echo '<meta property="og:type" content="product"/>' . "\n";
            echo '<meta property="og:price:amount" content="' . esc_attr($this->metaData['og:price:amount']) . '"/>' . "\n";
            echo '<meta property="og:price:currency" content="' . esc_attr($this->metaData['og:price:currency']) . '"/>' . "\n";
            $typeChanged= true;
        }else
        {
            return false;
        ...
        // Optional Output availability
        if (isset($this->metaData['product:availability']) && !empty($this->metaData['product:availability'])) {
            echo '<meta property="product:availability" content="' . esc_attr($this->metaData['product:availability']) . '"/>' . "\n";
        }
