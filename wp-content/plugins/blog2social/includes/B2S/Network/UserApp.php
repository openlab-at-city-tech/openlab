...
class B2S_Network_UserApp {
    ...
    public function getItemHtml($data = array()) {
        ...
        foreach ($data as $networkId => $network_apps) {
            ...
            $html .= '<span id="b2s-user-app-count-current" class="b2s-user-app-count-current" data-network-count-trigger="true" data-network-id="' . esc_attr($networkId) . '">' . esc_html($appCount) . '</span>/<span id="b2s-network-app-full-count">' . esc_html($this->allowedAppsPerNetwork[$networkId]) . '</span></span>';
            $html .= '</li>';
