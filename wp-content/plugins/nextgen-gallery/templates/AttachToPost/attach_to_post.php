...
<html>
	...
	<body>
		<div id="attach_to_post_tabs">
			<div class='ui-tabs-icon'><img src='<?php echo esc_url( $logo ); ?>' class='attach_to_post_logo'>
				<ul>
					<?php foreach ( $tabs as $id => $tab_params ) : ?>
						<li>
							<a href='#<?php echo esc_attr( $id ); ?>'>
								<?php echo esc_html( $tab_params['title'] ); ?>
							</a>
						</li>
					<?php endforeach ?>
				</ul>
			...
			<?php reset( $tabs ); foreach ( $tabs as $id => $tab_params ) : ?>
				<div class="main_menu_tab" id="<?php echo esc_attr( $id ); ?>">
					<?php
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Content is generated by internal settings forms
