 ...
 */
function badgeos_award_steps_ui_admin_scripts() {
	...
	if ( trim( $post_type ) === $settings['points_main_post_type'] ) {
		wp_enqueue_script( 'badgeos-award-steps-ui', $GLOBALS['badgeos']->directory_url . 'js/award-steps-ui.js', array( 'jquery-ui-sortable' ), BadgeOS::$version, true );
	}
}
add_action( 'admin_print_scripts-post-new.php', 'badgeos_award_steps_ui_admin_scripts', 11 );
add_action( 'admin_print_scripts-post.php', 'badgeos_award_steps_ui_admin_scripts', 11 );
...
function badgeos_add_award_steps_ui_meta_box() {
	...
	add_meta_box(
		...
		apply_filters(
			...
			esc_html__( 'Point Award', 'badgeos' )
		),
		'badgeos_award_steps_ui_meta_box',
		trim( $settings['points_main_post_type'] ),
		'advanced',
		'high'
	);
}
add_action( 'add_meta_boxes', 'badgeos_add_award_steps_ui_meta_box' );
...
function badgeos_award_steps_ui_meta_box( $post = null ) {
	...
	foreach ( $required_steps as $required_step ) {
		$required_step->order = get_award_step_menu_order( $required_step->ID );
	}
	...
	echo '<p>' . esc_html__( 'Defined points will be awarded when any of the following "steps" will be considered as complete. Use the "Label" field to optionally customize the titles of each step.', 'badgeos' ) . '</p>';

	/**
	 * Concatenate our step output.
	 */
	echo '<ul id="award_steps_list">';
	foreach ( $required_steps as $step ) {
		badgeos_award_steps_ui_html( $step->ID, $post->ID );
	}
	echo '</ul>';
	 ...
	 * Render our buttons.
	 */
	echo '<input style="margin-right: 1em" class="button" type="button" onclick="badgeos_add_new_award_step(' . esc_attr( $post->ID ) . ');" value="' . esc_attr( apply_filters( 'badgeos_award_steps_ui_add_new', esc_html__( 'Add New Step', 'badgeos' ) ) ) . '">';
	echo '<input class="button-primary" type="button" onclick="badgeos_update_award_steps();" value="' . esc_attr( apply_filters( 'badgeos_award_steps_ui_save_all', esc_html__( 'Save All Steps', 'badgeos' ) ) ) . '">';
	echo '<img class="save-steps-spinner save-award-steps-spinner" src="' . esc_url( admin_url( '/images/wpspin_light.gif' ) ) . '" style="margin-left: 10px; display: none;" />';
 ...
 */
function badgeos_award_steps_ui_html( $step_id = 0, $post_id = 0 ) {
	...
	<li class="step-row step-<?php echo esc_attr( $step_id ); ?>" data-step-id="<?php echo esc_attr( $step_id ); ?>">
		...
		<a class="delete-step" href="javascript: badgeos_delete_award_step( <?php echo esc_attr( $step_id ); ?> );"><?php esc_html_e( 'Delete', 'badgeos' ); ?></a>
		<input type="hidden" name="post_id" value="<?php echo esc_attr( $post_id ); ?>" />
		<input type="hidden" name="order" value="<?php echo esc_attr( get_award_step_menu_order( $step_id ) ); ?>" />

		<?php echo esc_html( apply_filters( 'badgeos_award_steps_ui_html_require_text', esc_html__( 'Require', 'badgeos' ), $step_id, $post_id ) ); ?>

		<?php do_action( 'badgeos_award_steps_ui_html_after_require_text', $step_id, $post_id ); ?>

		<select class="select-trigger-type" data-step-id="<?php echo esc_attr( $step_id ); ?>">
			<?php
			foreach ( get_badgeos_points_award_activity_triggers() as $value => $label ) {
				if ( is_array( $label ) ) {
		...
		<?php do_action( 'badgeos_award_steps_ui_html_after_trigger_type', esc_attr( $step_id ), $post_id ); ?>
		...
		if ( count( $dynamic_triggers ) > 0 ) {
			foreach ( $dynamic_triggers as $key => $data ) {
				$fields_group = array();
				?>
		<div id="badgeos_awardpoint_step_dynamic_section_<?php echo esc_attr( $key ); ?>" style="display:inline-block">
			<select id="badgeos_awardpoint_step_ddl_dynamic_<?php echo esc_attr( $key ); ?>" data-trigger="<?php echo esc_attr( $key ); ?>" class="badgeos_awardpoint_step_fields badgeos_awardpoint_step_ddl_dynamic" name="badgeos_awardpoint_step_ddl_dynamic_<?php echo esc_attr( $key ); ?>">
				<?php
				...
				foreach ( $fields_group as $fields_groupkey => $fields ) {
					foreach ( $fields as $fieldkey => $field ) {
						...
						switch ( trim( $field['type'] ) ) {
							case 'select':
								echo '<select id="' . esc_attr( $field['id'] ) . '" class="badgeos_awardpoint_step_fields badgeos_awardpoint_step_subddl_dynamic badgeos_awardpoint_step_fields_' . esc_attr( $fields_groupkey ) . ' badgeos_awardpoint_step_subddl_' . esc_attr( $fields_groupkey ) . '" name="' . esc_attr( $field['id'] ) . '">';
								...
								break;
							case 'text':
								echo '<input value="' . esc_attr( $sel_val ) . '" type="' . esc_attr( $field['type'] ) . '" size="4" id="' . esc_attr( $field['id'] ) . '"  class="badgeos_awardpoint_step_fields badgeos_awardpoint_step_subtxt_dynamic badgeos_awardpoint_step_fields_' . esc_attr( $fields_groupkey ) . ' badgeos_awardpoint_step_subtxt_' . esc_attr( $fields_groupkey ) . '" name="' . esc_attr( $field['id'] ) . '" />';
								break;
							case 'number':
								echo '<input value="' . esc_attr( $sel_val ) . '" type="' . esc_attr( $field['type'] ) . '" size="4" step="1" min="0" id="' . esc_attr( $field['id'] ) . '" class="badgeos_awardpoint_step_fields badgeos_awardpoint_step_subtxt_dynamic badgeos_awardpoint_step_fields_' . esc_attr( $fields_groupkey ) . ' badgeos_awardpoint_step_subtxt_' . esc_attr( $fields_groupkey ) . '" name="' . esc_attr( $field['id'] ) . '" />';
								break;
						}
					}
				}
		...
		?>
		<?php do_action( 'badgeos_award_steps_ui_html_after_dynamic_trigger_type', $step_id, $post_id ); ?>

		<select class="select-achievement-type select-achievement-type-<?php echo esc_attr( $step_id ); ?>">
		<?php
		foreach ( $achievement_types as $achievement_type ) {
			if ( trim( $settings['points_award_post_type'] ) === $achievement_type ) {
				continue;
			}
		...
		<?php do_action( 'badgeos_award_steps_ui_html_after_achievement_type', $step_id, $post_id ); ?>
		...
		<?php do_action( 'badgeos_award_steps_ui_html_after_achievement_post', $step_id, $post_id ); ?>
		<?php do_action( 'badgeos_steps_ui_html_before_paward_visit_post', $step_id, $post_id ); ?>
		...
		<?php do_action( 'badgeos_steps_ui_html_after_paward_visit_post', $step_id, $post_id ); ?>
		...
		<input type="number" size="5" min="0" placeholder="<?php esc_attr_e( 'Years', 'badgeos' ); ?>" 
		value="<?php echo esc_attr( intval( $requirements['num_of_years'] ) > 0 ? intval( $requirements['num_of_years'] ) : '1' ); ?>" class="badgeos-num-of-years badgeos-num-of-years-<?php echo esc_attr( $step_id ); ?>">
		...
		<input type="number" size="5" min="1" placeholder="<?php esc_attr_e( 'Months', 'badgeos' ); ?>" value="<?php esc_attr( intval( $requirements['num_of_months'] ) > 0 ? intval( $requirements['num_of_months'] ) : '1' ); ?>" class="badgeos-num-of-months badgeos-num-of-months-<?php echo esc_attr( $step_id ); ?>">
		...
		<?php echo esc_html( apply_filters( 'badgeos_award_steps_ui_html_count_text', esc_html__( 'time(s).', 'badgeos' ), $step_id, $post_id ) ); ?>

		<?php do_action( 'badgeos_award_steps_ui_html_after_count_text', $step_id, $post_id ); ?>

		<div class="step-title"><label for="step-<?php echo esc_attr( $step_id ); ?>-title"><?php esc_attr_e( 'Label', 'badgeos' ); ?>:</label> <input type="text" name="step-title" id="step-<?php echo esc_attr( $step_id ); ?>-title" class="title" value="<?php echo esc_attr( get_the_title( $step_id ) ); ?>" /></div>
 ...
 * @return mixed|void
 */
function badgeos_get_award_step_requirements( $step_id = 0 ) {
	...
	if ( ! empty( $requirements['achievement_type'] ) ) {
		$connected_activities = @get_posts(
			array(
				...
				'connected_type'   => $requirements['achievement_type'] . '-to-point_type',
	 ...
	 */
	return apply_filters( 'badgeos_get_award_step_requirements', $requirements, $step_id );
}
...
function badgeos_add_award_step_ajax_handler() {
	...
	$step_id = wp_insert_post(
		array(
			'post_type'   => trim( $settings['points_award_post_type'] ),
			'post_status' => 'publish',
		)
	);
	...
	$p2p_id = p2p_create_connection(
		'point_award-to-' . $achievement->post_type,
		array(
			'from' => $step_id,
...
add_action( 'wp_ajax_add_award_step', 'badgeos_add_award_step_ajax_handler' );
...
add_action( 'wp_ajax_delete_award_step', 'badgeos_delete_award_step_ajax_handler' );
 ...
 * @return void
 */
function badgeos_update_award_steps_ajax_handler() {
			...
			switch ( $trigger_type ) {
				...
				case 'badgeos_on_the_first_x_users':
					...
					if ( ! empty( $x_number_of_users ) ) {
						$title = sprintf( esc_html__( 'The first %d user(s)', 'badgeos' ), $x_number_of_users );
					} else {
				...
				case 'badgeos_points_on_birthday':
					$title = esc_html__( 'Points on User Birthday', 'badgeos' );
					break;
				case 'specific-achievement':
					p2p_create_connection(
						$step['achievement_type'] . '-to-point_type',
						array(
...
add_action( 'wp_ajax_update_award_steps', 'badgeos_update_award_steps_ajax_handler' );
 ...
 * @return void
 */
function badgeos_activity_trigger_post_award_select_ajax_handler() {
...
add_action( 'wp_ajax_post_award_select_ajax', 'badgeos_activity_trigger_post_award_select_ajax_handler' );
 ...
 */
function badgeos_get_p2p_award_id_from_child_id( $child_id = 0 ) {
 ...
 */
function get_award_step_menu_order( $step_id = 0 ) {
 ...
 */
function badgeos_compare_award_step_order( $step1 = 0, $step2 = 0 ) {
