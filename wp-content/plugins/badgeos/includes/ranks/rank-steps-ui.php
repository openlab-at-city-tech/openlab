...
function badgeos_rank_req_steps_ui_admin_scripts() {
	...
	if ( in_array( trim( $post_type ), badgeos_get_rank_types_slugs(), true ) ) {
		wp_enqueue_script( 'badgeos-rank-req-steps-ui', $GLOBALS['badgeos']->directory_url . 'js/rank-req-steps-ui.js', array( 'jquery-ui-sortable' ), Badgeos::$version, true );
	}
}
...
function badgeos_add_ranks_award_steps_ui_meta_box() {
	...
	if ( count( $rank_types ) ) {
		add_meta_box( 'badgeos_rank_req_steps_ui', apply_filters( 'badgeos_rank_req_steps_ui_title', esc_html__( 'Rank Requirement', 'badgeos' ) ), 'badgeos_rank_req_steps_ui_meta_box', $rank_types, 'advanced', 'high' );
	}
}
...
function badgeos_rank_req_steps_ui_meta_box( $post = null ) {
	 ...
	 */
	if ( trim( $settings['ranks_step_post_type'] ) === $post->post_type || in_array( $post->post_type, badgeos_get_rank_types_slugs(), true ) ) {
		if ( ( badgeos_get_rank_priority( $post->ID ) < 1 ) ) {

			echo '<p>' .
				 esc_html__( 'The rank with the lowest priority is set as default rank for all users so this rank should be created without requirements.', 'badgeos' )
				 . '<br>'
				 . esc_html__( 'You will be able to set requirements on next ranks. After save this rank, if it is not configured as the lowest priority you will be able to edit the requirements again.', 'badgeos' )
				 . '</p>';
			return;
		}
	}
	...
	echo '<p>' . esc_html__( 'Define the required "steps" for this rank to be considered complete. Use the "Label" field to optionally customize the titles of each step.', 'badgeos' ) . '</p>';
	 ...
	 * Render our buttons.
	 */
	echo '<input style="margin-right: 1em" class="button" type="button" onclick="badgeos_add_new_rank_req_step(' . esc_attr( $post->ID ) . ');" value="' . esc_attr( apply_filters( 'badgeos_rank_req_steps_ui_add_new', esc_html__( 'Add New Step', 'badgeos' ) ) ) . '">';
	echo '<input class="button-primary" type="button" onclick="badgeos_update_rank_steps();" value="' . esc_attr( apply_filters( 'badgeos_rank_req_steps_ui_save_all', esc_html__( 'Save All Steps', 'badgeos' ) ) ) . '">';
	echo '<img class="save-steps-spinner save-ranks-steps-spinner" src="' . esc_url( admin_url( '/images/wpspin_light.gif' ) ) . '" style="margin-left: 10px; display: none;" />';
...
function badgeos_rank_req_steps_ui_html( $step_id = 0, $post_id = 0 ) {
	...
	<li class="step-row step-<?php echo esc_attr( $step_id ); ?>" data-step-id="<?php echo esc_attr( $step_id ); ?>">
		...
		<a class="delete-step" href="javascript: badgeos_delete_rank_req_step( <?php echo esc_attr( $step_id ); ?> );"><?php esc_html_e( 'Delete', 'badgeos' ); ?></a>
		<input type="hidden" name="post_id" value="<?php echo esc_attr( $post_id ); ?>" />
		<input type="hidden" name="order" value="<?php echo esc_attr( get_ranks_step_menu_order( $step_id ) ); ?>" />

		<?php echo esc_html( apply_filters( 'badgeos_rank_req_steps_ui_html_require_text', esc_html__( 'Require', 'badgeos' ), $step_id, $post_id ) ); ?>
		...
		<input type="number" size="5" min="1" placeholder="<?php esc_attr_e( 'Years', 'badgeos' ); ?>" value="<?php esc_attr( intval( $requirements['num_of_years'] ) > 0 ? intval( $requirements['num_of_years'] ) : '1' ); ?>" class="badgeos-num-of-years badgeos-num-of-years-<?php echo esc_attr( $step_id ); ?>">
		<?php do_action( 'badgeos_rank_steps_ui_html_after_num_of_years', $step_id, $post_id ); ?>

		<input type="number" size="5" min="0" placeholder="<?php esc_attr_e( 'X Users', 'badgeos' ); ?>" value="<?php echo esc_attr( intval( $requirements['x_number_of_users'] ) > 0 ? intval( $requirements['x_number_of_users'] ) : '0' ); ?>" class="badgeos-x-number-of-users badgeos-x-number-of-users-<?php echo esc_attr( $step_id ); ?>">
		...
		<input type="number" size="5" min="1" placeholder="<?php esc_attr_e( 'Days', 'badgeos' ); ?>" value="<?php esc_attr( intval( $requirements['num_of_days'] ) > 0 ? intval( $requirements['num_of_days'] ) : '1' ); ?>" class="badgeos-num-of-days badgeos-num-of-days-<?php echo esc_attr( $step_id ); ?>">
		<?php do_action( 'badgeos_rank_steps_ui_html_after_num_of_days', $step_id, $post_id ); ?>

		<input class="required-count" type="text" size="3" maxlength="3" value="<?php echo esc_attr( $total_count ); ?>" placeholder="1">
		<?php echo esc_html( apply_filters( 'badgeos_rank_req_steps_ui_html_count_text', esc_html__( 'time(s).', 'badgeos' ), $step_id, $post_id ) ); ?>
...
function badgeos_add_rank_req_step_ajax_handler() {
	 ...
	 */
	badgeos_rank_req_steps_ui_html( $step_id, absint( $_POST['achievement_id'] ) );
...
function badgeos_rank_activity_trigger_post_award_select_ajax_handler() {
	...
	$exclude_posts = isset( $_REQUEST['excluded_posts'] ) ? array_map( 'sanitize_text_field', (array) $_REQUEST['excluded_posts'] ) : array();

	$requirements = isset( $_REQUEST['step_id'] ) ? badgeos_get_rank_req_step_requirements( absint( $_REQUEST['step_id'] ) ) : 0;
