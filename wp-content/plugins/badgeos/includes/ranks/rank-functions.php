...
/**
 * Return true if post_type is related to rank.
 *
 ...
 * @param string $rank_type rank type.
 *
 * @return mixed|void.
 */
 ...
 * @param int $point_type point_type.
 *
 * @return bool|mixed|void.
 */
function badgeos_get_ranks_by_unlock_points( $user_id, $points, $point_type = 0 ) {
	...
	global $wpdb;
	$unlockable_ranks = array();
	$ranks_types      = badgeos_get_rank_types_slugs();
	if ( count( $ranks_types ) > 0 ) {
		...
		$ranks = $wpdb->get_results(
			$wpdb->prepare(
                ...
                where p.post_status = 'publish' AND pm.meta_value = 'Yes' and p.post_type in ( %s )
                ",
				$formatted_ranks_types
			)
		);
		foreach ( $ranks as $rank ) {
			...
			if ( ! badgeos_user_has_a_rank( $post_id, $user_id ) 
				&& absint( $rank_point['_ranks_points_to_unlock'] ) <= $points
			 	&& $rank_point['_ranks_points_to_unlock_type'] == $point_type ) {
				$unlockable_ranks[] = array(
					'rank_id'        => $post_id,
					'credits'        => absint( $rank_point['_ranks_points_to_unlock'] ),
					'credit_type_id' => absint( $rank_point['_ranks_points_to_unlock_type'] ),
				);
			}
		}
 ...
 *
 * @param null $rank_type rank type.
 *
 * @return mixed|void.
 */
 ...
 * @param int   $new_rank_id new rank id.
 * @param array $args args.
 *
 * @return bool..
 */
...
function badgeos_update_user_rank( $args = array() ) {
	...
	$rank_entry_id = badgeos_add_rank(
		array(
...
function badgeos_earn_achivement_by_rank( $user_id, $new_rank, $old_rank, $admin_id, $achievement_id ) {
	...
	if ( count( $achievement_types ) ) {
		...
		$ranks = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT p.ID as achievement_id, pvalue.meta_value as rank_id 
                    FROM $wpdb->postmeta AS pm
                    ...
                    where p.post_status = 'publish' AND pm.meta_value = 'rank' and p.post_type in ( %s )",
				implode( "','", $achievement_types )
			)
		);
		foreach ( $ranks  as $rank ) {
...
function badgeos_get_rank_based_achievements() {
	...
	if ( empty( $achievements ) ) {
		...
		$achievements = $wpdb->get_results(
            ...
            LEFT JOIN $wpdb->postmeta AS m1 ON ( posts.ID = m1.post_id AND m1.meta_key = %s )
            LEFT JOIN $wpdb->postmeta AS m2 ON ( posts.ID = m2.post_id AND m2.meta_key = %s )
...
function badgeos_get_user_ranks( $args = array() ) {
	...
	if ( $args['total_only'] === true ) {
		$user_ranks = $wpdb->get_var(
			$wpdb->prepare( 'SELECT count(id) as id FROM %s WHERE %s', $table_name, $where )
		);
	} else {
...
function badgeos_get_rank_priority( $rank_id = 0 ) {
	...
	if ( badgeos_get_post_field( 'post_status', $rank_id ) === 'auto-draft' ) {
		...
		$last = $wpdb->get_var(
			$wpdb->prepare(
			'SELECT p.menu_order FROM %1s AS p WHERE p.post_type = %s AND p.post_status = %s ORDER BY menu_order DESC LIMIT 1',
				$wpdb->posts,
				$rank_type,
				'publish'
			)
...
function badgeos_get_rank_types_slugs_detailed() {
	...
	foreach ( $rank_types as $rtype ) {
		...
		$main[ $rtype->post_name ] = array(
			'singular_name' => $rtype->post_title,
	...
	if ( is_array( $rank_types ) && ! empty( $rank_types ) ) {
		...
		foreach ( $rank_types as $key => $rank_type ) {
			if ( $key === $selected || 'all' === $selected ) {
				$ranks_to_award = new WP_Query(
					array(
						'post_type'    	 => $key,
						'post__not_in' 	 => $earned_ranks,
						'posts_per_page' => 100
					)
				);
...
function badgeos_get_rank_image( $rank_id = 0, $rank_width = '', $rank_height = '' ) {
	...
	$user_earned_ranks = badgeos_get_user_ranks(
		array(
			'user_id' => get_current_user_id(),
			'rank_id' => $rank_id,
		)
...
function badgeos_ranks_set_default_thumbnail( $post_id ) {
	...
	if ( empty( $thumbnail_id ) ) {
		...
		if ( empty( $thumbnail_id ) || is_wp_error( $thumbnail_id ) ) {
			...
			badgeos_is_rank( $post_id )
			&& is_object( $rank )
			&& ! get_post_thumbnail_id( $rank->ID )
		) {
			set_post_thumbnail( $rank->ID, $thumbnail_id );
			badgeos_flush_rewrite_rules();
		}
	}
