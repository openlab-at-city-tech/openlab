...
class P2P_Box {
	...
	public function init_scripts() {
		...
		wp_register_script(
			'mustache',
			plugins_url( 'mustache.js', __FILE__ ),
			array(),
			'0.7.2',
			true
		);
		...
		wp_enqueue_script(
			'p2p-box',
			plugins_url( 'box.js', __FILE__ ),
			array( 'backbone', 'mustache' ),
			P2P_PLUGIN_VERSION,
			true
		);
	...
	protected function render_create_connections( $item ) {
		...
		// Search tab
		$tab_content = P2P_Mustache::render(
			'tab-search',
			array(
	...
	protected function candidate_row( $item ) {
		...
		$title_data = array_merge(
			$this->columns['title']->get_data( $item ),
			array(
				'title'   => $title,
				'item-id' => $item->get_id(),
			)
		...
		die(
			json_encode(
				$this->candidate_rows(
					...
					sanitize_text_field( $_REQUEST['paged'] ),
