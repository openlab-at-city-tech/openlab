...
_ready(function () {
    body = document.body;
});_N2.d('SmartSliderWidgetThumbnailDefaultVertical', 'SmartSliderWidget', function () {
    "use strict";
     ...
     */
    function SmartSliderWidgetThumbnailDefaultVertical(slider, parameters) {
    ...
    SmartSliderWidgetThumbnailDefaultVertical.prototype = Object.create(_N2.SmartSliderWidget.prototype);
    SmartSliderWidgetThumbnailDefaultVertical.prototype.constructor = SmartSliderWidgetThumbnailDefaultVertical;

    SmartSliderWidgetThumbnailDefaultVertical.prototype.onStart = function () {
    ...
    SmartSliderWidgetThumbnailDefaultVertical.prototype.onBeforeShow = function () {
        ...
        for (var i = 0; i < realSlides.length; i++) {
            var dot = scroller.querySelector('[data-slide-public-id="' + _NodeGetData(realSlides[i].element, 'slidePublicId') + '"]');
    ...
    SmartSliderWidgetThumbnailDefaultVertical.prototype.onWidgetsReady = function () {
    ...
    SmartSliderWidgetThumbnailDefaultVertical.prototype.onScroll = function () {

        var scrollTop = this.bar.scrollTop,
            height = this.bar.clientHeight,
            scrollHeight = this.bar.scrollHeight;

        _NodeSetData(this.widget, 'hasPrevious', scrollTop === 0 ? 0 : 1);
        _NodeSetData(this.widget, 'hasNext', scrollTop + height < scrollHeight ? 1 : 0);
    };

    var isFired = false;
    SmartSliderWidgetThumbnailDefaultVertical.prototype.onDotClick = function (slide) {
    ...
    SmartSliderWidgetThumbnailDefaultVertical.prototype.onDotHover = function (slide) {
        this.slider.directionalChangeToReal(slide.index);
    };

    SmartSliderWidgetThumbnailDefaultVertical.prototype.onSlideSwitch = function (e) {
        ...
        var thumbnailDotCS = window.getComputedStyle(targetSlide.thumbnailDot),
            dotOffsetTop = targetSlide.thumbnailDot.offsetTop - parseInt(thumbnailDotCS.getPropertyValue('margin-top')),
            dotHeight = targetSlide.thumbnailDot.getBoundingClientRect().height + parseInt(thumbnailDotCS.getPropertyValue('margin-top')) + parseInt(thumbnailDotCS.getPropertyValue('margin-bottom')),
            scrollTop = this.bar.scrollTop,
            style = window.getComputedStyle(this.bar),
            paddingTop = parseInt(style.paddingTop),
            height = this.bar.clientHeight,
            scrollHeight = this.bar.scrollHeight;

        if (dotOffsetTop < scrollTop || dotOffsetTop + dotHeight > scrollTop + height) {
            this.bar.scrollTop = Math.min(scrollHeight - height, -paddingTop + dotOffsetTop);
        }
    };

    SmartSliderWidgetThumbnailDefaultVertical.prototype.activateDots = function (currentSlide) {
    ...
    SmartSliderWidgetThumbnailDefaultVertical.prototype.previousPane = function () {

        this.bar.scrollTop -= this.bar.clientHeight * 0.75;
    };

    SmartSliderWidgetThumbnailDefaultVertical.prototype.nextPane = function () {

        this.bar.scrollTop += this.bar.clientHeight * 0.75;
    };

    SmartSliderWidgetThumbnailDefaultVertical.prototype.getSize = function () {
        return this.getWidth();
    };

    return SmartSliderWidgetThumbnailDefaultVertical;
});})(window);