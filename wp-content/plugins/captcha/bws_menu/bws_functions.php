...
* General functions for mysimplewp plugins
...
if ( ! function_exists ( 'bws_form_restore_default_settings' ) ) {
	function bws_form_restore_default_settings( $plugin_basename, $change_permission_attr = '' ) { ?>
		<form method="post" action="">
			<p><?php _e( 'Restore all plugin settings to defaults', 'mysimplewp' ); ?></p>
			<p>
				<input <?php echo $change_permission_attr; ?> type="submit" class="button" value="<?php _e( 'Restore settings', 'mysimplewp' ); ?>" />
			</p>
 ...
 * Function to add 'mysimplewp' slug for SW_Menu MO file if BWS_Menu loaded from theme.
...
if ( ! function_exists ( 'bws_get_mofile' ) ) {
	...
	function bws_get_mofile( $mofile, $domain ) {
		...
		if ( 'mysimplewp' == $domain ) {

			$locale = get_locale();

			return str_replace( $locale, "mysimplewp-{$locale}", $mofile );
...
if ( isset( $bws_menu_source ) && 'themes' == $bws_menu_source ) {
	...
	load_theme_textdomain( 'mysimplewp', get_stylesheet_directory() . '/inc/bws_menu/languages' );

	remove_filter( 'load_textdomain_mofile', 'bws_get_mofile' );

} else {

	load_plugin_textdomain( 'mysimplewp', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
			...
			if ( is_plugin_active( $plugin_basename ) ) {
				...
				wp_die(
					...
					sprintf(
						...
						__( 'requires', 'mysimplewp' ),

						$require_wp,

						__( 'or higher, that is why it has been deactivated! Please upgrade WordPress and try again.', 'mysimplewp' ),

						__( 'Back to the WordPress', 'mysimplewp' ),

						$admin_url,

						__( 'Plugins page', 'mysimplewp' )
	...
	function bws_plugin_reviews_block( $plugin_name, $plugin_slug ) { ?>
		...
		<div class="bws-plugin-reviews">
			...
			<div class="bws-plugin-reviews-rate">
				...
				<?php _e( 'Like the plugin?', 'mysimplewp' ); ?>
		...
		if ( isset( $bstwbsftwppdtplgns_options['wrong_license_key'][ $plugin_key ] ) ) {
			...
			echo '<tr class="bws-plugin-update-tr plugin-update-tr ' . $class . '" id="' . $explode_plugin_key[0] . '-update" data-slug="' . $explode_plugin_key[0] . '" data-plugin="' . $plugin_key . '">
					...
					<td colspan="' . $wp_list_table->get_column_count() . '" class="plugin-update colspanchange">
						...
						echo '<strong>' . __( 'WARNING: Illegal use notification', 'mysimplewp' ) . '.</strong> ' . __( 'You can use one license of the Pro plugin for one domain only. Please check and edit your license or domain if necessary using you personal Client Area. We strongly recommend you to solve the problem within 24 hours, otherwise the Pro plugin will be deactivated.', 'mysimplewp' ) . ' <a target="_blank" href="http://mysimplewp.com/contact-us/">' . __( 'Learn More', 'mysimplewp' ) . '</a>';
			...
			echo '<tr class="bws-plugin-update-tr plugin-update-tr ' . $class . '" id="' . $explode_plugin_key[0] . '-update" data-slug="' . $explode_plugin_key[0] . '" data-plugin="' . $plugin_key . '">
					...
					<td colspan="' . $wp_list_table->get_column_count() . '" class="plugin-update colspanchange">
						...
						if ( isset( $bstwbsftwppdtplgns_options['trial'][ $plugin_key ] ) && $link_slug != false ) {
							...
							echo __( 'Notice: Your Pro Trial license has expired. To continue using the plugin, you should buy a Pro license', 'mysimplewp' ) . ' - <a href="http://mysimplewp.com/contact-us/' . $link_slug .'/">http://mysimplewp.com/contact-us/</a>';

						} else {

							echo __( 'Your license has expired. To continue getting top-priority support and plugin updates, you should extend it.', 'mysimplewp' ) . ' <a target="_new" href="http://mysimplewp.com/contact-us">' . __( "Learn more", 'mysimplewp' ) . '</a>';
			...
			echo '<tr class="bws-plugin-update-tr plugin-update-tr ' . $class . '" id="' . $explode_plugin_key[0] . '-update" data-slug="' . $explode_plugin_key[0] . '" data-plugin="' . $plugin_key . '">
					...
					<td colspan="' . $wp_list_table->get_column_count() . '" class="plugin-update colspanchange">
						...
						if ( $free_plugin_name != false ) {
							...
							echo sprintf( __( 'Notice: You are using the Pro Trial license of %s plugin.', 'mysimplewp' ), $free_plugin_name );

						} else {

							_e( 'Notice: You are using the Pro Trial license of plugin.', 'mysimplewp' );

						}

						if ( isset( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) )

							echo ' ' . __( "The Pro Trial license will expire on", 'mysimplewp' ) . ' ' . $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] . '.';
			...
			foreach ( $bws_versions_notice_array as $key => $value ) { ?>
				...
				<div class="update-nag"><?php
					...
					echo sprintf(
							...
							__( 'or higher! We do not guarantee that our plugin will work correctly. Please upgrade to WordPress latest version.', 'mysimplewp' )
				...
				<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
					...
					<div class="bws_banner_on_plugin_page bws_banner_to_settings">
						...
						<div class="text">
							...
							<strong><?php printf( __( 'Thank you for installing %s plugin!', 'mysimplewp' ), $bws_plugin_banner_to_settings[0]['plugin_info']['Name'] ); ?> </strong><br />

							<?php _e( "Let's get started", 'mysimplewp' ); ?>:

							<a href="<?php echo $bws_plugin_banner_to_settings[0]['settings_url']; ?>"><?php _e( 'Settings', 'mysimplewp' ); ?></a>

							<?php if ( false != $bws_plugin_banner_to_settings[0]['post_type_url'] ) { ?>

								<?php _e( 'or', 'mysimplewp' ); ?>

								<a href="<?php echo $bws_plugin_banner_to_settings[0]['post_type_url']; ?>"><?php _e( 'Add New', 'mysimplewp' ); ?></a>
						...
						<form action="" method="post">
							...
							<button class="notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'mysimplewp' ); ?>"></button>
					...
					<div class="bws_banner_on_plugin_page bws_banner_to_settings_joint">
						...
						<form action="" method="post">
							...
							<div class="bws-text">
								...
								<strong><?php _e( 'Thank you for installing plugins by mysimplewp!', 'mysimplewp' ); ?></strong>

								<div class="hide-if-no-js bws-more-links">

									<a href="#" class="bws-more"><?php _e( 'More Details', 'mysimplewp' ); ?></a>

									<a href="#" class="bws-less hidden"><?php _e( 'Less Details', 'mysimplewp' ); ?></a>
							...
							<div class="bws-details hide-if-js">
								...
								<?php foreach ( $bws_plugin_banner_to_settings as $value ) { ?>
									...
									<div>
										...
										<strong><?php echo str_replace( ' by mysimplewp', '', $value['plugin_info']['Name'] ); ?></strong>&ensp;<a href="<?php echo $value['settings_url']; ?>"><?php _e( 'Settings', 'mysimplewp' ); ?></a>

										<?php if ( false != $value['post_type_url'] ) { ?>

											&ensp;|&ensp;<a target="_blank" href="<?php echo $value['post_type_url']; ?>"><?php _e( 'Add New', 'mysimplewp' ); ?></a>
		...
		*/
		...
		if ( ! empty( $bstwbsftwppdtplgns_options['deprecated_function'] ) ) { ?>
			...
			<div class="update-nag">
				...
				<strong><?php _e( 'Deprecated function(-s) is used on the site here:', 'mysimplewp' ); ?></strong>
				...
				<?php _e( 'This function(-s) will be removed over time. Please update the product(-s).', 'mysimplewp' ); ?>
			...
			if ( $this_banner == $value[0] ) {
				...
				<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
					...
					<div class="<?php echo $this_banner_prefix; ?>_message bws_banner_on_plugin_page bws_go_pro_banner" style="display: none;">
						...
						<button class="<?php echo $this_banner_prefix; ?>_close_icon close_icon notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'mysimplewp' ); ?>"></button>
						...
						<div class="text"><?php
							...
							_e( 'It’s time to upgrade your', 'mysimplewp' ); ?> <strong><?php echo $plugin_info['Name']; ?> plugin</strong> <?php _e( 'to', 'mysimplewp' ); ?> <strong>Pro</strong> <?php _e( 'version!', 'mysimplewp' ); ?><br />

							<span><?php _e( 'Extend standard plugin functionality with new great options.', 'mysimplewp' ); ?></span>

						</div>

						<div class="button_div">

							<a class="button" target="_blank" href="http://mysimplewp.com"><?php _e( 'Learn More', 'mysimplewp' ); ?></a>
		...
		if ( isset( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) && ( strtotime( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) < strtotime( date("m/d/Y") . '+1 month' ) ) && ( strtotime( $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ) > strtotime( date("m/d/Y") ) ) ) {
			...
			<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
				...
				<div class="<?php echo $plugin_prefix; ?>_message_timeout bws_banner_on_plugin_page bws_banner_timeout" style="display:none;">
					...
					<button class="<?php echo $plugin_prefix; ?>_close_icon close_icon notice-dismiss bws_hide_settings_notice" title="<?php _e( 'Close notice', 'mysimplewp' ); ?>"></button>
					...
					<div class="text"><?php printf( __( "Your license key for %s expires on %s and you won't be granted TOP-PRIORITY SUPPORT or UPDATES.", 'mysimplewp' ), '<strong>' . $plugin_name . '</strong>', $bstwbsftwppdtplgns_options['time_out'][ $plugin_key ] ); ?> <a target="_new" href="http://mysimplewp.com/contact-us/"><?php _e( "Learn more", 'mysimplewp' ); ?></a></div>
		...
		<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">
			...
			<div class="bws_banner_on_plugin_page bws_suggest_feature_banner">
				...
				<div class="text">
					...
					<strong><?php printf( __( 'Thank you for choosing %s plugin!', 'mysimplewp' ), $plugin_info['Name'] ); ?></strong><br />

					<?php _e( "If you have a feature, suggestion or idea you'd like to see in the plugin, we'd love to hear about it!", 'mysimplewp' ); ?>

					<a target="_blank" href="http://mysimplewp.com/contact-us"><?php _e( 'Suggest a Feature', 'mysimplewp' ); ?></a>
	...
	function bws_show_settings_notice() { ?>
		...
		<div id="bws_save_settings_notice" class="updated fade below-h2" style="display:none;">
			...
			<p>
				...
				<strong><?php _e( 'Notice', 'mysimplewp' ); ?></strong>: <?php _e( "The plugin's settings have been changed.", 'mysimplewp' ); ?>

				<a class="bws_save_anchor" href="#bws-submit-button"><?php _e( 'Save Changes', 'mysimplewp' ); ?></a>
...
if ( ! function_exists( 'bws_hide_premium_options' ) ) {
	...
	function bws_hide_premium_options( $options ) {
		...
		return array(
				...
				'message' => __( 'You can always look at premium options by checking the "Pro Options" in the "Misc" tab.', 'mysimplewp' ),
		...
		if ( ! empty( $bws_shortcode_list ) ) { ?>
			...
			<script type='text/javascript'>
				...
				var bws_shortcode_button = {
					...
					'label': '<?php esc_attr_e( "Add SW Shortcode", "mysimplewp" ); ?>',

					'title': '<?php esc_attr_e( "Add SW Plugins Shortcode", "mysimplewp" ); ?>',
			...
			<?php if ( isset( $post_type ) && in_array( $post_type, array( 'post', 'page' ) ) ) {
				...
				$tooltip_args = array(
					...
					'content' 		=> '<h3>' . __( 'Add shortcode', 'mysimplewp' ) . '</h3><p>' . __( "Add mysimplewp plugins' shortcodes using this button.", 'mysimplewp' ) . '</p>',
...
if ( ! function_exists ( 'bws_form_restore_default_confirm' ) ) {
	...
	function bws_form_restore_default_confirm( $plugin_basename ) { ?>
		...
		<div>
			...
			<p><?php _e( 'Are you sure you want to restore default settings?', 'mysimplewp' ) ?></p>

			<form method="post" action="">

				<p>

					<button class="button button-primary" name="bws_restore_confirm"><?php _e( 'Yes, restore all settings', 'mysimplewp' ) ?></button>

					<button class="button" name="bws_restore_deny"><?php _e( 'No, go back to the settings page', 'mysimplewp' ) ?></button>
			...
			<div id="bws_shortcode_popup" style="display:none;">
				...
				<div id="bws_shortcode_popup_block">
					...
					<div id="bws_shortcode_select_plugin">
						...
						<h4><?php _e( 'Plugin', 'mysimplewp' ); ?></h4>
					...
					<div id="bws_shortcode_content">
						...
						<h4><?php _e( 'Shortcode settings', 'mysimplewp' ); ?></h4>
					...
					<div id="bws_shortcode_content_bottom">
						...
						<p><?php _e( 'The shortcode will be inserted', 'mysimplewp' ); ?></p>
					...
					<?php if ( $wp_version < '3.9' ) { ?>
						...
						<p>
							...
							<button class="button-primary primary bws_shortcode_insert"><?php _e( 'Insert', 'mysimplewp' ); ?></button>
...
if ( ! function_exists( 'bws_help_tab' ) ) {
	...
	function bws_help_tab( $screen, $args ) {
		...
		$url = ( ! empty( $args['section'] ) ) ? 'http://mysimplewp.com/contact-us/' . $args['section'] : 'http://mysimplewp.com';



		$content = '<p><a href="' . $url . '" target="_blank">' . __( 'Visit Help Center', 'mysimplewp' ) . '</a></p>';
		...
		$screen->set_help_sidebar(
			...
			'<p><strong>' . __( 'For more information:', 'mysimplewp' ) . '</strong></p>' .

			'<p><a href="https://drive.google.com/folderview?id=0B5l8lO-CaKt9VGh0a09vUjNFNjA&usp=sharing#list" target="_blank">' . __( 'Documentation', 'mysimplewp' ) . '</a></p>' .

			'<p><a href="http://www.youtube.com/user/mysimplewp/playlists?flow=grid&sort=da&view=1" target="_blank">' . __( 'Video Instructions', 'mysimplewp' ) . '</a></p>' .

			'<p><a href="http://mysimplewp.com/contact-us" target="_blank">' . __( 'Submit a Request', 'mysimplewp' ) . '</a></p>'
