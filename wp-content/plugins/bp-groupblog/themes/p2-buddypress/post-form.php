	...
	<form id="new_post" name="new_post" method="post" action="<?php echo site_url(); ?>" style="display:none;" />
		...
		<div id="whats-new-type"	
			<ul id="post-types">
				...
				<li><a id="post"<?php if ( $_GET['p'] == 'post' ) : ?> class="selected"<?php endif; ?> href="<?php echo site_url( '?p=post' ) ?>" title="<?php _e( 'Post', 'p2' ) ?>"><?php _e( 'Post', 'p2' ) ?></a></li>
				<li><a id="photo"<?php if ( $_GET['p'] == 'photo' ) : ?> class="selected"<?php endif; ?> href="<?php echo site_url( '?p=photo' ) ?>" title="<?php _e( 'Photo', 'p2' ) ?>"><?php _e( 'Photo', 'p2' ) ?></a></li>
				<li><a id="video"<?php if ( $_GET['p'] == 'video' ) : ?> class="selected"<?php endif; ?> href="<?php echo site_url( '?p=video' ) ?>" title="<?php _e( 'Video', 'p2' ) ?>"><?php _e( 'Video', 'p2' ) ?></a></li>
				
				<?php if ( groups_is_user_admin( bp_loggedin_user_id(), bp_get_group_id() ) || groups_is_user_mod( bp_loggedin_user_id(), bp_get_group_id() ) ) : ?>
					<li><a id="featured"<?php if ( $_GET['p'] == 'featured' ) : ?> class="selected"<?php endif; ?> href="<?php echo site_url( '?p=featured' ) ?>" title="<?php _e( 'Featured', 'p2' ) ?>"><?php _e( 'Featured', 'p2' ) ?></a></li>
				<?php endif; ?>
			</ul>
		</div>	
