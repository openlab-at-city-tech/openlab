...
if ( isset ( $demo_links[ $page_slug ] ) ) :
    ...
    <div class="col-auto">
        <a class="btn btn-default" title="<?php esc_attr_e( 'View this page at Bookly Pro Demo', 'bookly' ) ?>"
                ...
                <?php endif ?>
        >
            <i class="fas fa-fw fa-certificate"></i><span class="d-none d-lg-inline ms-2"><?php esc_html_e( 'View this page at Bookly Pro Demo', 'bookly' ) ?></span>
        </a>
        <?php if ( ! $dismiss ) : ?>
...
?>
    <div class="col-auto">
        <a href="<?php echo esc_url( 'https://hub.bookly.pro/go/' . $page_slug ) ?>" id="bookly-help-btn" target="_blank" class="btn btn-default" title="<?php esc_attr_e( 'Documentation', 'bookly' ) ?>">
            <i class="far fa-question-circle"></i><span class="d-none d-lg-inline ms-2"><?php esc_html_e( 'Documentation', 'bookly' ) ?></span>
        </a>
    </div>
    ...
    <div class="col-auto">
        <a href="#bookly-contact-us-modal" id="bookly-contact-us-btn" class="btn btn-default" title="<?php esc_attr_e( 'Contact us', 'bookly' ) ?>" data-toggle="bookly-modal"
                <?php if ( $show_contact_us_notice ) : ?>
                    ...
                    data-content="<?php echo esc_attr( '<button type="button" class="close ms-2"><span>&times;</span></button>' . __( 'Need help? Contact us here.', 'bookly' ) ) ?>"
                <?php endif ?>
        >
            <i class="fas fa-headset"></i><span class="d-none d-lg-inline ms-2"><?php esc_html_e( 'Contact us', 'bookly' ) ?></span>
        </a>
...
?>
    <div class="col-auto">
        <a class="btn btn-default" title="<?php esc_attr_e( 'Feature requests', 'bookly' ) ?>"
                ...
                <?php endif ?>
        >
            <i class="far fa-lightbulb"></i><span class="d-none d-lg-inline ms-2"><?php esc_html_e( 'Feature requests', 'bookly' ) ?></span>
        </a>
        <?php if ( ! $dismiss ) : ?>
    ...
    <div class="col-auto">
        <a href="<?php echo Utils\Common::prepareUrlReferrers( Urls::REVIEWS_PAGE, 'feedback' ) ?>" id="bookly-feedback-btn" target="_blank" class="btn btn-default" title="<?php esc_attr_e( 'Feedback', 'bookly' ) ?>"
                <?php if ( $show_feedback_notice ) : ?>
                    data-content="<?php echo esc_attr( '<button type="button" class="close ms-2"><span>&times;</span></button>' . __( 'We care about your experience of using Bookly!<br/>Leave a review and tell others what you think.', 'bookly' ) ) ?>"
                <?php endif ?>
        >
            <i class="far fa-comment-dots"></i><span class="d-none d-lg-inline ms-2"><?php esc_html_e( 'Feedback', 'bookly' ) ?></span>
        </a>
    </div>
