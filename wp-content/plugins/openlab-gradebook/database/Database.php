<?php
class ANGB_DATABASE{
	const oplb_gradebook_db_version = 3.2;	
	...
	public function database_alter(){
		...
		global $wpdb;		
		if(get_site_option( 'oplb_gradebook_db_version' )==2){
			$sql = "ALTER TABLE oplb_gradebooks CHANGE name name MEDIUMTEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, 
			...
			$wpdb->query($sql);		
			update_option( "oplb_gradebook_db_version", 3 );				
		}		
		if(get_site_option( 'oplb_gradebook_db_version' )==3){
			$sql = "ALTER TABLE oplb_assignment CHANGE assign_points_earned assign_points_earned decimal(7,2) NOT NULL";
			$wpdb->query($sql);		
			update_option( "oplb_gradebook_db_version", 3.1 );				
		}
		if(get_site_option( 'oplb_gradebook_db_version' )==3.1){
			$gradebooks = $wpdb ->get_results("SELECT * FROM oplb_gradebooks", ARRAY_A); 			
			$assignments = $wpdb -> get_results("SELECT * FROM oplb_assignments", ARRAY_A); 
			$cells = $wpdb -> get_results("SELECT * FROM oplb_assignment", ARRAY_A);
			foreach ($gradebooks as $gradebook){
				...
				foreach($assignments_temp as &$assignment){
					$amid = $assignment['id'];					
					$wpdb->update('oplb_assignments', array( 'assign_order' => $i), array('id' => $amid));   
					...
					foreach($cells_temp as &$cell){
						$cid = $cell['id'];
						$wpdb->update('oplb_assignment', array( 'assign_order' => $i), array('id' => $cid));   						
					}
					$i++;
				}				
			}
			update_option( "oplb_gradebook_db_version", 3.14 );				
		}
		if(get_site_option( 'oplb_gradebook_db_version' )==3.14){ 
			$sql = 'ALTER TABLE oplb_assignments ADD assign_visibility VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT "Students"';					
			$wpdb->query($sql);				
		    update_option( "oplb_gradebook_db_version", 3.141);
		}	
		if(get_site_option( 'oplb_gradebook_db_version' )==3.141){ 
			$sql1 = "SELECT uid FROM oplb_gradebook";					
			...
			$sql1 = "DELETE FROM oplb_gradebook WHERE uid IN (".implode(',', $uids_to_delete_from_gradebook).")";
			$sql2 = "DELETE FROM oplb_assignment WHERE uid IN (".implode(',', $uids_to_delete_from_gradebook).")";			
			$wpdb->query($sql1);
			$wpdb->query($sql2);			
		    update_option( "oplb_gradebook_db_version", 3.1415);
		}	
		if(get_site_option( 'oplb_gradebook_db_version' )==3.1415){ 
			$sql = "RENAME TABLE oplb_gradebooks TO oplb_gradebook_courses, oplb_gradebook TO oplb_gradebook_students, oplb_assignments TO oplb_gradebook_assignments, oplb_assignment TO oplb_gradebook_cells";																
			$wpdb->query($sql);	
	  		$db_name = 'oplb_gradebook_users';
			...
			$sql = 'INSERT INTO oplb_gradebook_users (uid, gbid) SELECT uid, gbid FROM oplb_gradebook_students';
			$wpdb->query($sql);			
			$sql = 'SELECT gbid FROM oplb_gradebook_courses';
			$gbids = $wpdb -> get_col('SELECT id FROM oplb_gradebook_courses');
			$sql_sub = '';
			foreach($gbids as $gbid){
				$sql_sub = $sql_sub. "(1,$gbid,'instructor'),";
			}
			$sql_sub = rtrim($sql_sub, ',');
			$sql = 'INSERT INTO oplb_gradebook_users (uid, gbid, role) VALUES '. $sql_sub;
			$wpdb -> query($sql);
			$sql = 'DROP TABLE oplb_gradebook_students';
			$wpdb -> query($sql);		
	...
	public function database_init() {
		global $wpdb;
	  	$db_name = 'oplb_gradebook_courses';
		if($wpdb->get_var('SHOW TABLES LIKE "'.$db_name.'"') != $db_name){
			$sql = 'CREATE TABLE ' . $db_name . ' (
			id int(11) NOT NULL AUTO_INCREMENT,
		...
		$db_name2 = 'oplb_gradebook_assignments';
		...
		if($wpdb->get_var('SHOW TABLES LIKE "'.$db_name2.'"') != $db_name2){
			...
			$oplb_assignments_columns = $wpdb->get_col( "SELECT column_name FROM information_schema.columns
				WHERE table_name = 'oplb_gradebook_assignments' ORDER BY ordinal_position" );
			$missing_columns = array_diff($table_columns, $oplb_assignments_columns);
			if(count($missing_columns)){
				//add missing columns
				$sql = 'ALTER TABLE oplb_gradebook_assignments ';
