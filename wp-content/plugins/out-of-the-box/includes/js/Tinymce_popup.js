jQuery(document).ready(function ($) {
  ...
  $('.outofthebox  .insert_shortcode_cf').click(function (event) {
    insertOutoftheBoxShortCodeCF(event)
  });
  ...
  $('.outofthebox .list-container').on('click', '.entry_media_shortcode', function (event) {
    insertOutoftheBoxShortCodeMedia($(this).closest('.entry'));
  });
  ...
  function insertOutoftheBoxShortCodeCF(event) {
    event.preventDefault();
    ...
    if (data !== false) {
      ...
      $('.outofthebox-shortcode-value', window.parent.document).val(encoded_data);
      window.parent.jQuery('.outofthebox-shortcode-value').trigger('change');

      if (data.indexOf('userfolders="auto"') > -1) {
        $('.out-of-the-box-upload-folder', window.parent.document).fadeIn();
      } else {
        $('.out-of-the-box-upload-folder', window.parent.document).fadeOut();
      }
  ...
  function insertOutoftheBoxShortCodeMedia($entry_element) {

    $("#OutoftheBox_showplaylist").prop("checked", false);
    var file_name = $entry_element.find('.entry_name a:first').data('filename');
