  ...
  var outofthebox_wc = {
    ...
    afterFileSelected: function (event) {
      ...
      if ($(outofthebox_wc.lastSelectedButton).closest('.downloadable_files').length > 0) {
        ...
        fileRow.find('.file_url > input').val('https://dropbox.com/' + decodeURIComponent(event.data.entry_id) + outofthebox_woocommerce_translation.download_url + encodeURIComponent(Base64.encode(event.data.entry_id)) + '&account_id=' + event.data.account_id);
      ...
      if ($(outofthebox_wc.lastSelectedButton).closest('.wc-product-document').length > 0) {
        ...
        row.find('.wc-product-document-file-location input:first').val(outofthebox_woocommerce_translation.wcpd_url + encodeURIComponent(Base64.encode(event.data.entry_id)) + '&account_id=' + event.data.account_id);
      }
      /* END Support for WooCommerce Product Documents */
