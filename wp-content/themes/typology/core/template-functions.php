...
if ( !function_exists( 'typology_get_social_share' ) ):
    function typology_get_social_share() {
        ...
        $title = typology_esc_text( wp_strip_all_tags( get_the_title() ) );
...
if ( !function_exists( 'typology_get_author_links' ) ):
    function typology_get_author_links( $author_id ) {
        ...
        if ( is_singular() ) {
            $url = typology_is_co_authors_active() ? get_author_posts_url( $author_id->ID, $author_id->user_nicename ) : get_author_posts_url( get_the_author_meta( 'ID', $author_id ) );
            $output .= '<a class="typology-button-social hover-on" href="' . esc_url( $url ) . '">' . __typology( 'view_all' ) . '</a>';
        }

        $url = typology_is_co_authors_active() ? get_the_author_meta( 'url', $author_id->ID ) : get_the_author_meta( 'url', $author_id );
