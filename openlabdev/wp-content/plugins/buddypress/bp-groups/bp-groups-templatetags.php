...
class BP_Groups_Template {
	...
	function bp_groups_template( $user_id, $type, $page, $per_page, $max, $slug, $include, $search_terms, $populate_extras ) {
		global $bp;
		...
		if ( 'invites' == $type ) {
			...
			$this->groups = groups_get_groups( array( 'type' => $type, 'per_page' => $this->pag_num, 'page' =>$this->pag_page, 'user_id' => $user_id, 'include' => $include, 'search_terms' => $search_terms, 'populate_extras' => $populate_extras ) );
		}
		//print_r($this->groups);
		if ( 'invites' == $type ) {
...
function bp_has_groups( $args = '' ) {
	...
	$defaults = array(
		...
		'include' => $include, // pass group ids
	...
	$groups_template = new BP_Groups_Template( (int)$user_id, $type, (int)$page, (int)$per_page, (int)$max, $slug, $include, $search_terms, (bool)$populate_extras );
