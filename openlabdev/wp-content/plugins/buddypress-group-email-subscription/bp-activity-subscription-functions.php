<?php
//WDS MODIFIED
// replace "group" tect with $group_type cookie
...
function ass_group_subscribe_settings ( $group = false ) {
	...
	<b><?php _e('How do you want to read this '.$group_type.'?', 'bp-ass'); ?></b>
	...
	<div class="ass-email-explain"><?php _e('I will read this '.$group_type.' on the web', 'bp-ass'); ?></div>
	</div>
	...
	<div class="ass-email-explain"><?php _e('Send all '.$group_type.' activity as it arrives', 'bp-ass'); ?></div>
	</div>
...
//	$group_type=$_COOKIE["wds_bp_group_type"];
	...
	if ( $bp->current_component == 'groups' && $bp->current_action == 'notifications' ) {
		...
		if ( isset( $_POST['ass-save'] ) ) {
			...
			bp_core_add_message( sprintf( __( 'Your email notifications are set to %s for this %s.', 'bp-ass' ), ass_subscribe_translate( $action ), $group_type ) );
...
function ass_default_subscription_settings_form() {
	...
	<p><?php _e('When new users join this '.$group_type.', their default email notification settings will be:', 'bp-ass'); ?></p>
	<div class="radio">
		<label><input type="radio" name="ass-default-subscription" value="no" <?php //ass_default_subscription_settings( 'no' ) ?> /> 
			<?php _e( 'No Email (users will read this '.$group_type.' on the web - good for any '.$group_type.' - the default)', 'bp-ass' ) ?></label>
		<label><input type="radio" name="ass-default-subscription" value="sum" <?php //ass_default_subscription_settings( 'sum' ) ?> /> 
			<?php _e( 'Weekly Summary Email (the week\'s topics - good for large '.$group_type.'s)', 'bp-ass' ) ?></label>
		<label><input type="radio" name="ass-default-subscription" value="dig" <?php //ass_default_subscription_settings( 'dig' ) ?> /> 
			<?php _e( 'Daily Digest Email (all daily activity bundles in one email - good for medium-size '.$group_type.'s)', 'bp-ass' ) ?></label>
		<label><input type="radio" name="ass-default-subscription" value="sub" <?php //ass_default_subscription_settings( 'sub' ) ?> /> 
			<?php _e( 'New Topics Email (new topics are sent as they arrive, but not replies - good for small '.$group_type.'s)', 'bp-ass' ) ?></label>
		<label><input type="radio" name="ass-default-subscription" value="supersub" checked="checked" <?php //ass_default_subscription_settings( 'supersub' ) ?> /> 
			<?php _e( 'All Email (send emails about everything - recommended only for working '.$group_type.'s)', 'bp-ass' ) ?></label>
	</div>
	<hr />
	<?php
}
...
function ass_clean_subject( $subject ) {
	...
	$subject_quotes = preg_replace( '/posted on the discussion topic /', 'posted on the discussion topic "', $subject );
	$subject_quotes = preg_replace( '/started the discussion topic /', 'started the discussion topic "', $subject_quotes );	
	if ( $subject != $subject_quotes )
		$subject = preg_replace( '/ in the '.$group_type.' /', '" in the '.$group_type.' ', $subject_quotes );
...
function ass_add_notice_to_notifications_page() {
	$group_type=$_COOKIE["wds_bp_group_type"];
	echo '<p><b>'.__(ucfirst($group_type).' Email Settings','bp-ass').'</b></p>';
	echo '<p>' . sprintf( __('To change the email notification settings for your '.$group_type.'s go to %s and click change for each '.$group_type.'.','bp-ass') . '</p>', '<a href="'. bp_loggedin_user_domain() . trailingslashit( BP_GROUPS_SLUG ) . '">'.__('My '.$group_type,'bp-ass') .'</a>' );
}
...
function ass_admin_notice_form() {	
	...
	if ( groups_is_user_admin( $bp->loggedin_user->id , $bp->groups->current_group->id ) || is_site_admin() ) {
		$submit_link = $bp->root_domain . '/' . $bp->groups->slug . '/' . $bp->groups->current_group->slug . '/notifications';
		?> 
		<h3><?php _e('Send an email notice to everyone in the '.$group_type, 'bp-ass'); ?></h3>
		<p><?php _e('You can use the form below to send an email notice to all '.$group_type.' members.', 'bp-ass'); ?> <br>
		<b><?php _e('Everyone in the '.$group_type.' will receive the email -- regardless of their email settings -- so use with caution', 'bp-ass'); ?></b>.</p>
		<form action="<?php echo $submit_link ?>" method="post">
		...
		<input type="submit" name="ass_admin_notice_send" value="<?php _e('Email this notice to everyone in the '.$group_type, 'bp-ass') ?>" />
		</form> 
		<?php
	}
}
...
function ass_admin_notice() {
    ...
    if ( $bp->current_component == 'groups' && $bp->current_action == 'admin' && $bp->action_variables[0] == 'notifications' ) {
		...
		if ( isset( $_POST[ 'ass_admin_notice_send' ] ) && isset( $_POST[ 'ass_admin_notice' ] ) ) {
			...
			$subject .= __(' - sent from the '.$group_type.' ', 'bp-ass') . $group_name .' [' . get_blog_option( BP_ROOT_BLOG, 'blogname' ) . ']';
			$message = sprintf( __(
'This is a notice from the '.$group_type.' \'%s\':

"%s"


To view this '.$group_type.' log in and follow the link below:
%s
